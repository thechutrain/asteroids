...
class Asteroid {
  ...
  init() {
    ...
    let height = Asteroid.gameRef.canvasElem.height;
  ...
  containsPoint(ptCoord) {
    ...
    const { leftBound, rightBound, upperBound, lowerBound } = getBounds(this.currPoints);
    ...
    const lineResults = this.currPoints.map((pt1, index, coordArr) => {
      const x1 = pt1.x;
      const y1 = pt1.y;
      const x2 = index + 1 < coordArr.length ? coordArr[index + 1].x : coordArr[0].x;
      const y2 = index + 1 < coordArr.length ? coordArr[index + 1].y : coordArr[0].y;
      const m = (y1 - y2) / (x1 - x2);
      const b = y1 - m * x1;
